...
婚礼扫码页的留言后端：一个**只写不读**的 REST 入口。扫码页把朋友的祝福/留言 POST 到本函数的 **HTTP 访问服务（云接入）**地址，函数校验后写入云数据库 `guestbook` 集合。新人到 CloudBase 控制台查看/导出留言，**没有任何 API 能读**——天然实现「只收集、不回显」。
...
| 字段 | 说明 |
|------|------|
| `type` | `blessing`（公开祝福）\| `letter`（解锁后留言，匿名） |
| `entryId` | 扫码条目 id（如 `A-01`） |
| `to` | 收件人（公开区为空；解锁后留言归属该收件人） |
| `name` | 留言者名字（公开祝福选填；解锁后留言匿名，恒为空） |
| `text` | 留言正文（必填，≤500 字） |
| `createdAt` | 服务端时间 |
