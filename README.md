...
**启用后页面效果**：**无收件人条目**的照片/描述下方出现「💌 给新人的祝福」（填名字选填 + 留言必填）；**有收件人的门禁条目**扫码后不显示祝福框，答对收信码的信件视图信末出现**匿名「给新人留言」框**（无名字输入，占位「给新人留言」）。提交后经云函数直写云数据库；**页面不回显他人留言**——你在 CloudBase 控制台「云开发 → 数据库 → guestbook 集合」里查看，或点「导出」。
