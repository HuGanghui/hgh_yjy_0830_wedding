...
**启用后页面效果**：**无收件人条目**的照片/描述下方出现匿名「给新人留言」框；**有收件人的门禁条目**扫码后不显示留言框，答对收信码的信件视图信末出现同格式「给新人留言」框。两处均匿名（无名字输入）、占位「给新人留言」、按钮「💌提交」。提交后经云函数直写云数据库；**页面不回显他人留言**——你在 CloudBase 控制台「云开发 → 数据库 → guestbook 集合」里查看，或点「导出」。
...
> ⚠️ **视频进度条限制**：CloudBase 默认域名不支持 HTTP Range（见「本地验证」），线上视频拉不动进度条；GitHub Pages 镜像支持 Range、可正常看视频。

**视频进度条限制的解决方案（2026-08-19 评估记录，暂未实现）**

根因：`tcbgw` 网关对 Range 请求返回 200 全量（上游 COS 本身支持 Range）。**下载走普通 GET、不走 Range**，所以不受此限制——这是方案①能生效的原理。

| 方案 | 效果 | 成本 / 可行性 |
|------|------|------|
| **① 视频下载按钮**（短期推荐） | 下载完整文件后，在手机本地播放器里正常看 + 拖进度；顺带满足宾客收藏视频 | 纯前端，立即可做。⚠️ 微信内置浏览器会拦网页下载（`<a download>` 也被拦，视频没法像图片那样「长按保存」）→ 需降级：提示「右上角 → 在浏览器打开」或引导到镜像页 |
| **② GitHub Pages 镜像链接**（短期推荐） | 镜像站 `https://huganghui.github.io/hgh_yjy_0830_wedding` 支持 Range，页内可正常看视频 | 零后端，页面加一个「视频不能拖进度？点此到镜像页观看」链接即可 |
| **③ 绑自定义 CDN 域名**（根治） | 走 CDN 后 Range 正常，一劳永逸；还能换掉超长的 `*.tcloudbaseapp.com` 域名 | 需已备案域名 + ICP 备案（约 1~3 周），婚礼临近时来不及，适合婚后逐步上线 |
| fetch 全量 → Blob 播放 | 页内可 seek | ❌ 大视频先整段下载，手机端卡，不推荐 |
| 视频挪到云存储 | 未知（COS 支持 Range 但未必绕得过网关） | ❌ 偏离现架构，不值得现在动 |

**建议**：短期上 ① + ② 组合（视频下载按钮 + 镜像页链接），根治等婚后绑自定义域名（③）。
